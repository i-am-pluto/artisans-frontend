...
const SearchResult = () => {
  ...
  useEffect(() => {
    const setSearchResults = async () => {
      const response = await fetch(
        `https://artisans-and-co.onrender.com:5000/api/search/${pgeno}?` +
          new URLSearchParams({
            query: query,
          }),
        {
  ...
  const getProductListPLtoH = async () => {
    const response = await fetch(
      `https://artisans-and-co.onrender.com:5000/api/search/${pgeno}/plth?` +
        new URLSearchParams({
          query: query,
        }),
      {
  ...
  const getProductListPHtoL = async () => {
    const response = await fetch(
      `https://artisans-and-co.onrender.com:5000/api/search/${pgeno}/phtl?` +
        new URLSearchParams({
          query: query,
        }),
      {
  ...
  useEffect(() => {
    const setBeforeAfter = async (pg) => {
      if (pg < 0) {
        ...
        const response = await fetch(
          `https://artisans-and-co.onrender.com:5000/api/search/${pg - 1}?` +
            new URLSearchParams({
              query: query,
            }),
          {
